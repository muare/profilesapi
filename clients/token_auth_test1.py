...
def client_registration():
    data= {"username":"wangkaiyu","password1":"xintent123","password2":"xintent123"}
...
def client_login():
    ...
    response = requests.post("http://localhost:8000/api/rest-auth/login/",data=data)
    print(response.status_code)
    print(response.content)
...
def client_update_profile():    
    ...
    data = {"city": "hangzhou"}
    response = requests.put("http://localhost:8000/api/profiles/5/",headers=headers,data=data)
    print(response.status_code)
    print(json.loads(response.content))
